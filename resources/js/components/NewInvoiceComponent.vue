  ...
  <div>
    <div class="card">
      <div class="card-header">
        <h3 class="mb-5">Create New Invoice</h3>
        ...
        <div class="row">
          <div class="col-4">
            ...
            <div v-if="client.id">
              <h5>{{ client.business_name }} <i class="fa fa-pencil text-danger cpointer" @click="selectClient()"></i></h5>
              {{ client.office_address }}<br />
              {{ client.website }}
            </div>
          </div>
          <div class="col-4"></div>
          <div class="col-4">
            ...
            <div v-if="company.id">
              <h5>{{ company.name }} <i class="fa fa-pencil text-danger cpointer" @click="selectCompany()"></i></h5>
              {{ company.mobile }}<br />
              {{ company.address }}<br />
              {{ company.email }}
            </div>
            ...
            <div class="form-group row mt-3">
              ...
              <div class="col-md-8">
                ...
                <select v-model="form.vendor" id="vendor_id" class="form-control">
                  ...
                  <option v-for="(vendor, index) in vendors" :key="index + '-' + vendor.id" :value="vendor.id">
                    {{ vendor.name }}
                  </option>
                </select>
              </div>
            ...
            <div class="form-group row mt-3">
              ...
              <div class="col-md-8">
                <select v-model="form.refer" id="refer_id" class="form-control">
                  ...
                  <option v-for="(refer, index) in clients" :key="index + '-' + refer.id" :value="refer.id">
                    {{ refer.business_name }}
                  </option>
                </select>
              </div>
        ...
        <div class="row justify-content-between">
          ...
          <div class="col-3">
            ...
            <select class="form-control" @change="selectedService($event)">
              ...
              <option v-for="(service, index) in allServices" :key="index + '-' + service.id" :value="service.id">
                {{ service.name }}
              </option>
            </select>
          </div>
          ...
          <div class="modal-container">
            <div class="modal-body">
              ...
              <div v-if="selectClientBlock">
                <input type="text" class="form-control" v-model="search_client" placeholder="Search Client..." />
...
<script>
export default {
  // props: ['auth_user','currencySymbol'],
  data() {
    return {
      search_client: '',
      dialog: false,
  ...
  computed: {
    ...
    filteredClients() {
      return this.clients.filter(client => {
        return client.business_name.toLowerCase().includes(this.search_client.toLowerCase())
      })
    }
  },
  watch: {},
      ...
      axios
        ...
        .then(function (response) {
          if (response.status === 200) {
            ...
            vm.client.business_name = response.data.client.business_name;
            vm.client.office_address = response.data.client.office_address;
            vm.client.website = response.data.client.website;
            vm.showModal = false;
